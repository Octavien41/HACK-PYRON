...
Pyronear développe un model d'IA opensource qui détecte des feux de forêts avec des caméras placées à des points stratégiques. Le programme utilise YOLO pour la détéction de fumée. L'IA produit un score de confiance qui produit ensuite une alerte transmise à la police si ce score dépasse un seuil. Ce seuil se corrèle lui même avec la probabilité d'incendie qui est tirée d'un score : le Fire Warning Indice (FWI). Ce FWI est récupéré via une API. 
...
Ces deux sets contiennent des images de caméras détectant un feu. Pour le premier set, un parsing était nécessaire pour récupérer les données de l'image et de la caméra, pour le deuxième set, toutes les données étaient présentes dans le csv. Nous avons eu besoin des deux sets de données pour avoir un maximum d'informations.
...
---
Avec plus de temps on pourrait s'attaquer à un problème rencontré les jours nuageux. Ces jours sont très majoritairement classés en 'verylow' par le FWI. Or les nuages sont souvent très similaires à la fumée mais sur seulement quelques images.
